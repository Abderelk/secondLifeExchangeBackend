...
export const getConversations = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const conversations = await Conversation.find({
      ...
      .populate('exchange', 'status owner')
    ...
    const formattedConversations = conversations.map((conv) => {
      ...
      return {
        ...
        exchangeId: conv.exchange ? conv.exchange._id.toString() : null,
        exchangeStatus: conv.exchange ? conv.exchange.status : null,
        // isOwner = true si l'utilisateur est le propriétaire de l'item demandé (celui qui reçoit la demande)
        isOwner: conv.exchange ? conv.exchange.owner?.toString() === userId.toString() : false,
...
export const getMessages = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    let conversation = await Conversation.findOne({
      ...
      .populate('exchange', 'status owner requester')
    ...
    // Si pas d'exchange lié, essayer de le trouver
    let exchangeData = conversation.exchange;
    if (!exchangeData && conversation.itemRequested) {
      const foundExchange = await Exchange.findOne({
        $or: [
          { requestedItem: conversation.itemRequested._id, requester: { $in: conversation.participants.map(p => p._id) } },
          { requestedItem: conversation.itemRequested._id, owner: { $in: conversation.participants.map(p => p._id) } },
        ]
      }).sort({ createdAt: -1 }).lean();

      if (foundExchange) {
        console.log('🔗 Exchange trouvé par recherche:', foundExchange._id);
        exchangeData = {
          _id: foundExchange._id,
          status: foundExchange.status,
          owner: foundExchange.owner,
          requester: foundExchange.requester,
        } as any;

        // Mettre à jour la conversation pour le futur
        await Conversation.updateOne(
          { _id: conversationId },
          { $set: { exchange: foundExchange._id } }
        );
      }
    }
    ...
    console.log('📋 Is owner:', exchangeData ? exchangeData.owner?.toString() === currentUserId : false);
    ...
    res.json({
      ...
      data: {
        conversation: {
          ...
          exchangeId: exchangeData ? exchangeData._id.toString() : null,
          exchangeStatus: exchangeData ? exchangeData.status : null,
          isOwner: exchangeData ? exchangeData.owner?.toString() === currentUserId : false,
